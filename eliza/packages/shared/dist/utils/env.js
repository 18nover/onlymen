...
import { buildBrandEnvSyncAliases, normalizeBrandEnvPrefix, } from "../config/brand-env-aliases.js";
 ...
 */
export function readAliasedEnv(key) {
...
export function syncElizaEnvAliases(options = {}) {
    ...
    if (!env)
        return;
    const brandedPrefix = normalizeBrandEnvPrefix(options.brandedPrefix ?? DEFAULT_BRANDED_PREFIX);
    ...
    if (!env.ELIZA_APP_ROUTE_PLUGIN_MODULES) {
        env.ELIZA_APP_ROUTE_PLUGIN_MODULES = (options.appRoutePluginModules ?? DEFAULT_APP_ROUTE_PLUGIN_MODULES).join(",");
    }
}
