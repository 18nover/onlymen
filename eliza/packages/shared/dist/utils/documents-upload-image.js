...
const TARGET_DOCUMENT_IMAGE_BYTES = Math.floor(MAX_DOCUMENT_IMAGE_PROCESSING_BYTES * 0.9);
...
function browserCompressionPlatform() {
    return {
        ...
        loadImageSource: async (file) => {
            ...
            try {
                const image = await new Promise((resolve, reject) => {
                    ...
                    nextImage.onerror = () => reject(new Error(`Failed to load image "${file.name}"`));
...
function clampDimensions(width, height, maxDimension) {
...
function cloneUploadFile(file, blob) {
...
export function isDocumentImageFile(file) {
    if (file.type.startsWith("image/"))
        return true;
    const lowerName = file.name.toLowerCase();
    return IMAGE_EXTENSIONS.some((extension) => lowerName.endsWith(extension));
}
export async function maybeCompressDocumentUploadImage(file, platform = browserCompressionPlatform()) {
    if (!isDocumentImageFile(file) ||
    ...
    let { width, height } = clampDimensions(image.width, image.height, IMAGE_MAX_DIMENSION);
    let bestBlob = null;
    while (true) {
        ...
        const largestEdge = Math.max(width, height);
        if (largestEdge <= IMAGE_MIN_DIMENSION)
            break;
        const nextScale = Math.max(IMAGE_MIN_DIMENSION / largestEdge, IMAGE_SCALE_STEP);
        if (nextScale >= 1)
            break;
...
//# sourceMappingURL=documents-upload-image.js.map