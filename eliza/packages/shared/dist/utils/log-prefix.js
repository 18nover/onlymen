...
function prefixFromPackageName(name) {
...
export function getLogPrefix() {
    ...
    const nameArgMatch = runtimeProcess?.argv?.find((a) => a.startsWith("--name="));
    if (nameArgMatch) {
        const name = nameArgMatch.split("=")[1];
        cachedPrefix = `[${name}]`;
        return cachedPrefix;
    }
    const cwd = typeof runtimeProcess?.cwd === "function" ? runtimeProcess.cwd() : "";
