...
export class SandboxRegistry {
    ...
    async register() {
        ...
        logger.info(`[sandbox-registry] Registered ${this.config.serverName} -> ${this.config.serverUrl} (agent ${this.config.agentId}, ttl ${this.config.ttlSeconds}s, transport ${this.tcp ? "tcp" : "rest"})`);
    }
    async refresh() {
        await this.writeKeys();
    }
    async unregister() {
        ...
        const keysToDelete = [];
        if (registeredUrl === serverUrl)
            keysToDelete.push(serverUrlKey);
        if (registeredServer === serverName)
            keysToDelete.push(agentServerKey);
        if (keysToDelete.length > 0) {
            await this.command(["DEL", ...keysToDelete]);
        }
        logger.info(`[sandbox-registry] Unregistered ${serverName} (agent ${agentId})`);
    }
    startHeartbeat(intervalMs) {
        ...
        this.heartbeatTimer = setInterval(() => {
            void this.refresh().catch((err) => {
                logger.warn(`[sandbox-registry] Heartbeat refresh failed: ${formatErr(err)}`);
            });
        }, intervalMs);
        if (typeof this.heartbeatTimer === "object" &&
    ...
    async pipeline(commands) {
        ...
        if (!res.ok) {
            throw new Error(`Upstash pipeline failed: ${res.status} ${await res.text()}`);
        }
        const json = (await res.json());
    ...
    async tcpExec(commands) {
        ...
        if (password) {
            ...
            preamble.push(username ? ["AUTH", username, password] : ["AUTH", password]);
        }
        if (db)
            preamble.push(["SELECT", db]);
        ...
        return new Promise((resolve, reject) => {
            ...
            socket.setTimeout(REGISTRY_TCP_TIMEOUT_MS, () => finish(new Error("Redis TCP timeout")));
            ...
            socket.on("data", (chunk) => {
                ...
                const firstErr = parsed.replies.find((r) => r instanceof RespError);
                if (firstErr) {
...
function encodeRespCommands(commands) {
    const parts = [];
 ...
 */
function parseRespReplies(buffer, expected) {
    const replies = [];
    let offset = 0;
    const parseOne = () => {
        ...
        switch (type) {
            ...
            case 0x24: {
                ...
                if (buffer.length < end + 2)
                    return null; // bulk + trailing CRLF
                return {
            ...
            default:
                ...
                return {
                    value: new RespError(`unsupported RESP type ${String.fromCharCode(type)}`),
                    next: afterLine,
                };
        }
    };
 ...
 */
export function buildSandboxRegistryFromEnv(env = process.env, ttlSeconds = 90) {
    ...
    const agentId = env.SANDBOX_ROUTE_AGENT_ID?.trim() || env.SANDBOX_AGENT_ID?.trim();
