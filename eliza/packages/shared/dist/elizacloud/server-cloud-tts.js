...
export function __resetCloudBaseUrlCache() {
...
export function resolveElevenLabsApiKeyForCloudMode(env = process.env) {
    ...
    const cloudTtsEnabled = env.ELIZAOS_CLOUD_USE_TTS === "true" ||
...
export function ensureCloudTtsApiKeyAlias(env = process.env) {
    const directKey = normalizeSecretEnvValue(env.ELEVENLABS_API_KEY);
    if (directKey) {
        return false;
    }
...
export function resolveCloudTtsBaseUrl(env = process.env) {
 ...
 */
function resolveWwwApexBaseSiblings(base) {
...
export function resolveCloudTtsCandidateUrls(env = process.env) {
 ...
 * is queued.
 */
export function resolveCloudSttCandidateUrls(env = process.env) {
    const base = resolveCloudTtsBaseUrl(env).replace(/\/+$/, "");
    const candidates = new Set();
    for (const siblingBase of resolveWwwApexBaseSiblings(base)) {
 ...
 */
export function shouldRetryCloudTtsUpstream(status) {
...
export function resolveElizaCloudTtsVoiceId(bodyVoiceId, env = process.env) {
...
export function normalizeElizaCloudTtsModelId(raw) {
    const trimmed = raw.trim();
    if (!trimmed)
...
export function resolveCloudProxyTtsModel(bodyModel, env = process.env) {
...
export function _internalResolveCloudApiKey(env = process.env) {
    return resolveCloudApiKey(env);
}
//# sourceMappingURL=server-cloud-tts.js.map