...
export class StreamingEchoDelayCalibrator {
     ...
     */
    observe(nearPcm, farPcm) {
        if (this.locked || nearPcm.length === 0)
            return;
        let farEnergy = 0;
        for (let i = 0; i < farPcm.length; i++)
            farEnergy += farPcm[i] * farPcm[i];
        ...
        while (this.calSampleCount > ECHO_CAL_MAX_SAMPLES &&
            this.calNear.length > 1) {
            this.calSampleCount -= this.calNear.shift().length;
            this.calFar.shift();
        }
        if (this.calSampleCount < ECHO_CAL_TARGET_SAMPLES)
            return;
...
//# sourceMappingURL=delay-calibrator.js.map