...
export class NlmsEchoCanceller {
    ...
    x; // far-end ring buffer (most-recent-first)
    taps;
    mu;
    eps;
    delay;
    ...
    delayLine = [];
    xEnergy = 0; // running ‖x‖² over the active window (incremental)
    peakXEnergy = 0; // slowly-decaying envelope of ‖x‖² (far-end scale)
    pNear = 0; // smoothed near-end power (DTD)
    pFar = 0; // smoothed far-end reference power (DTD)
    hangover = 0; // samples to stay frozen after a double-talk trigger
    lastEchoPow = 0;
    ...
    process(nearEnd, farEnd) {
        ...
        for (let i = 0; i < n; i++) {
            ...
            const ref = this.delayLine.length > this.delay
                ? this.delayLine.shift()
                : 0;
            this.pushRef(ref);
            ...
            this.peakXEnergy = Math.max(this.peakXEnergy * NlmsEchoCanceller.PEAK_DECAY, this.xEnergy);
            // Estimate echo ŷ = wᵀ·x and the error e = d − ŷ (the cleaned sample).
            let yhat = 0;
            for (let k = 0; k < this.taps; k++)
                yhat += this.w[k] * this.x[k];
            const d = nearEnd[i];
            const e = d - yhat;
            ...
            const farActive = this.xEnergy > NlmsEchoCanceller.FAR_ACTIVITY_FRAC * this.peakXEnergy;
            ...
            if (!doubleTalk && farActive) {
                const reg = Math.max(this.eps, NlmsEchoCanceller.REG_FRAC * this.peakXEnergy);
                const norm = this.xEnergy + reg;
                const step = (this.mu * e) / norm;
                if (Number.isFinite(step)) {
                    for (let k = 0; k < this.taps; k++)
                        this.w[k] += step * this.x[k];
                }
            }
            ...
            const cleaned = this.resGain !== null &&
                !doubleTalk &&
                farActive &&
    ...
    observeFarEndSilence(nearEnd) {
        ...
        if (this.peakXEnergy < this.eps)
            this.peakXEnergy = 0;
        if (this.pFar < this.eps)
            this.pFar = 0;
        this.lastEchoPow = 0;
        this.lastResidualPow = residualPow / Math.max(1, n);
    }
    ...
    get lastErleDb() {
        if (this.lastResidualPow <= 0 || this.lastEchoPow <= 0)
            return 0;
    ...
    pushRef(sample) {
        ...
        if (this.xEnergy < 0)
            this.xEnergy = 0; // guard fp drift
        for (let k = this.taps - 1; k > 0; k--)
            this.x[k] = this.x[k - 1];
        this.x[0] = sample;
    }
}
...
function resolveResidualSuppressionGain(option) {
    if (!option)
        return null;
    if (option === true)
...
//# sourceMappingURL=nlms-echo-canceller.js.map