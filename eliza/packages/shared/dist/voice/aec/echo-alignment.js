...
function nccAtOffset(near, far, offset, minOverlap) {
    const overlap = Math.min(near.length, far.length - offset);
    if (overlap < minOverlap)
        return { corr: -1, overlap: Math.max(0, overlap) };
    let dot = 0;
    let nearEnergy = 0;
    let farEnergy = 0;
 ...
 */
export function estimateEchoAlignment(near, far, options = {}) {
    const minOverlap = Math.max(1, Math.floor(options.minOverlapSamples ?? 4000));
    const maxOffset = Math.max(0, Math.floor(options.maxOffsetSamples ?? far.length - minOverlap));
    ...
    for (let offset = 0; offset <= coarseMaxOffset; offset++) {
        const { corr } = nccAtOffset(nearCoarse, farCoarse, offset, coarseMinOverlap);
...
//# sourceMappingURL=echo-alignment.js.map