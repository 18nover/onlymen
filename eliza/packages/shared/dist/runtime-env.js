...
const DISABLE_AUTO_API_TOKEN_KEYS = ["ELIZA_DISABLE_AUTO_API_TOKEN"];
export const API_EXPOSE_PORT_KEYS = ["ELIZA_API_EXPOSE_PORT"];
const DESKTOP_API_PORT_KEYS = ["ELIZA_API_PORT", "ELIZA_PORT"];
...
export function resolveRuntimePorts(env = process.env) {
    return {
        serverOnlyPort: parsePositivePort(resolveEnvValue(env, "ELIZA_PORT")) ??
            parsePositivePort(resolveEnvValue(env, "ELIZA_UI_PORT")) ??
            DEFAULT_SERVER_ONLY_PORT,
        desktopApiPort: parsePositivePort(resolveEnvValue(env, "ELIZA_API_PORT")) ??
            parsePositivePort(resolveEnvValue(env, "ELIZA_PORT")) ??
            DEFAULT_DESKTOP_API_PORT,
        desktopUiPort: parsePositivePort(resolveEnvValue(env, "ELIZA_UI_PORT")) ??
...
export function isDevApiWatchEnabled(env = process.env, execArgv = process.execArgv) {
    return (execArgv.includes("--watch") ||
 ...
 */
export function resolveApiExposePort(env = process.env) {
...
export function isMobilePlatform(env = process.env) {
    const raw = resolvePlatform(env);
    if (!raw)
        return false;
    return MOBILE_PLATFORM_VALUES.has(raw);
}
export function isAndroidMobile(env = process.env) {
    return resolvePlatform(env) === "android";
}
export function isIosMobile(env = process.env) {
