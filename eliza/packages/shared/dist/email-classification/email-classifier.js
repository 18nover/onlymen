 ...
 */
import { logger, ModelType, parseJsonModelRecord, runWithTrajectoryPurpose, } from "@elizaos/core";
...
const UNSUB_FOOTER_RE = /\b(unsubscribe|manage (your )?subscription|email preferences)\b/i;
...
function cacheKey(messageId, model) {
    return `${messageId ?? "anon"}::${model}`;
}
function readSettingString(runtime, key) {
...
function readSettingBoolean(runtime, key) {
...
export function isEmailClassifierEnabled(runtime) {
    const setting = readSettingBoolean(runtime, SETTING_ENABLED);
    return setting ?? true;
}
export function getConfiguredEmailClassifierModel(runtime) {
    return readSettingString(runtime, SETTING_MODEL) ?? "TEXT_SMALL";
}
function lowerLocalPart(address) {
    if (!address)
        return null;
...
function hasUnsubscribeHeader(message) {
 ...
 */
export function classifyEmailByRules(message, opts = {}) {
    const signals = [];
    const subject = (message.subject ?? "").toLowerCase();
    const senderEmail = message.fromEmail?.toLowerCase() ??
...
function parseStructuredClassification(raw) {
...
function parseLlmClassification(raw) {
    const parsed = parseStructuredClassification(typeof raw === "string" ? raw : "");
    if (!parsed)
        return null;
    ...
    const confidenceValue = typeof confidenceRaw === "string" && confidenceRaw.trim().length > 0
...
export async function classifyEmail(runtime, message, opts = {}) {
    ...
    try {
        ...
        const raw = await runWithTrajectoryPurpose("lifeops-email-classifier", () => runtime.useModel(ModelType[modelKey], {
...
export function _resetEmailClassifierCache() {
