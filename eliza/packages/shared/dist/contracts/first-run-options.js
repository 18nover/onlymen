...
import { normalizeDeploymentTargetConfig, normalizeLinkedAccountFlagsConfig, normalizeServiceRoutingConfig, } from "./service-routing.js";
...
export const FIRST_RUN_PROVIDER_CATALOG = [
    ...
    },
    {
        ...
        keyPrefix: null,
        description: "Powers Codex-backed coding agents through the official Codex surface.",
        family: "openai",
        authMode: "subscription",
        group: "subscription",
        order: 30,
    ...
    },
    {
        ...
        keyPrefix: null,
        description: "Powers task agents through the authenticated Gemini CLI. No Gemini subscription token is imported into API env vars.",
        family: "gemini",
        authMode: "subscription",
        group: "subscription",
        order: 35,
    ...
    },
    {
        ...
        keyPrefix: null,
        description: "Stores z.ai Coding Plan credentials for the dedicated coding endpoint only, not the general z.ai API key path.",
        family: "zai",
        authMode: "subscription",
        group: "subscription",
        order: 36,
    ...
    },
    {
        ...
        pluginName: "@elizaos/plugin-openai",
        keyPrefix: null,
        description: "Stores Kimi Code credentials for Kimi's coding endpoint only, not the Moonshot general API key path.",
        family: "moonshot",
        authMode: "subscription",
        group: "subscription",
        order: 37,
    ...
    },
    {
        ...
        keyPrefix: null,
        description: "Unavailable until DeepSeek exposes a first-party coding subscription surface that can be integrated without API-key substitution.",
        family: "deepseek",
        authMode: "subscription",
        group: "subscription",
        order: 38,
...
function pickFirstRunCloudModelPreferences(value) {
    return {
...
function readFirstRunCloudModelPreferences(source) {
    if (!source) {
        return {};
    }
...
const FIRST_RUN_PROVIDER_ALIASES = {
...
export function getStoredSubscriptionProvider(selectionId) {
    return (SUBSCRIPTION_PROVIDER_SELECTIONS.find((provider) => provider.id === selectionId)?.storedProvider ?? "anthropic-subscription");
}
export function getStoredSubscriptionProviderForRequest(providerId) {
    const selection = normalizeSubscriptionProviderSelectionId(providerId);
    if (selection)
        return getStoredSubscriptionProvider(selection);
    if (typeof providerId !== "string")
        return null;
    const normalized = providerId.trim().toLowerCase();
    return (SUBSCRIPTION_PROVIDER_SELECTIONS.find((provider) => provider.storedProvider === normalized)?.storedProvider ?? null);
}
export function getSubscriptionProviderFamily(selectionId) {
    return (SUBSCRIPTION_PROVIDER_SELECTIONS.find((provider) => provider.id === selectionId)?.family ?? "anthropic");
}
export function requiresAdditionalRuntimeProvider(providerId) {
    const selection = normalizeSubscriptionProviderSelectionId(providerId);
    return Boolean(selection && selection !== "openai-subscription");
}
export function normalizeFirstRunProviderId(value) {
    ...
    for (const candidate of candidates) {
        const directMatch = FIRST_RUN_PROVIDER_CATALOG.find((provider) => provider.id === candidate);
        if (directMatch) {
    ...
    for (const candidate of candidates) {
        const pluginMatches = FIRST_RUN_PROVIDER_CATALOG.filter((provider) => provider.pluginName.toLowerCase() === candidate);
...
export function getFirstRunProviderOption(providerId) {
    const normalized = normalizeFirstRunProviderId(providerId);
    if (!normalized)
        return null;
    return (FIRST_RUN_PROVIDER_CATALOG.find((provider) => provider.id === normalized) ??
        null);
}
export function getFirstRunProviderFamily(providerId) {
    return getFirstRunProviderOption(providerId)?.family ?? null;
}
export function getStoredFirstRunProviderId(providerId) {
    const provider = getFirstRunProviderOption(providerId);
    if (!provider)
        return null;
    return provider.storedProvider ?? provider.id;
}
export function getDirectAccountProviderForFirstRunProvider(providerId) {
    const normalized = normalizeFirstRunProviderId(providerId);
    if (!normalized)
        return null;
    ...
    return DIRECT_ACCOUNT_PROVIDER_BY_FIRST_RUN_PROVIDER[normalized];
}
export function sortFirstRunProviders(providers) {
...
export function isFirstRunConnectionComplete(connection) {
...
function readFirstRunEnvContainer(config) {
...
export function getFirstRunProviderSignalEnvKeys(providerId) {
...
function readFirstRunProviderApiKey(config, providerId) {
    if (!providerId) {
        return undefined;
    }
    const provider = getFirstRunProviderOption(providerId);
...
function resolveLegacyDeploymentTargetInConfig(config) {
    ...
    const cloudProvider = normalizeFirstRunProviderId(readConfigString(cloud, "provider"));
...
export function resolveLinkedAccountsInConfig(config) {
    const root = asConfigRecord(config);
    const explicit = normalizeLinkedAccountFlagsConfig(root?.linkedAccounts) ?? {};
    const next = { ...explicit };
    const cloud = asConfigRecord(config?.cloud);
...
function deriveFirstRunConnectionFromRuntimeConfig(config) {
...
function resolveConfiguredLocalProviderFromSignals(config) {
    const agents = asConfigRecord(config?.agents);
    const defaults = asConfigRecord(agents?.defaults);
    const storedSubscriptionProvider = normalizeFirstRunProviderId(readConfigString(defaults, "subscriptionProvider"));
    ...
    for (const provider of FIRST_RUN_PROVIDER_CATALOG) {
        ...
        const detected = getFirstRunProviderSignalEnvKeys(providerId).some((key) => Boolean(readFirstRunEnvString(config, key)));
        if (detected) {
            return providerId;
        }
    }
...
export function normalizePersistedFirstRunConnection(value) {
...
export function normalizeFirstRunCredentialInputs(value) {
...
export function deriveFirstRunCredentialPersistencePlan(args) {
    const credentialInputs = normalizeFirstRunCredentialInputs(args.credentialInputs);
...
export function stripFirstRunConnectionSecrets(connection) {
    if (connection.kind === "cloud-managed") {
        return {
            kind: "cloud-managed",
            cloudProvider: "elizacloud",
...
export function inferCompatibilityFirstRunConnection(config) {
    const cloud = asConfigRecord(config?.cloud);
    const models = asConfigRecord(config?.models);
    const remoteApiBase = readConfigString(cloud, "remoteApiBase");
...
export function inferFirstRunConnectionFromConfig(config) {
...
//# sourceMappingURL=first-run-options.js.map