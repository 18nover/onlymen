 ...
 */
export function maxRawBytesForBase64(base64Cap) {
    return Math.floor(base64Cap / 4) * 3;
}
/** Largest raw image file whose base64 payload fits the image cap (~3.75 MiB). */
export const MAX_CHAT_IMAGE_RAW_BYTES = maxRawBytesForBase64(MAX_CHAT_IMAGE_BASE64_BYTES);
/** Largest raw non-image file whose base64 payload fits the media cap (~11.25 MiB). */
export const MAX_CHAT_MEDIA_RAW_BYTES = maxRawBytesForBase64(MAX_CHAT_MEDIA_BASE64_BYTES);
/**
...
export const CHAT_IMAGE_MIME_TYPE_SET = new Set(CHAT_IMAGE_MIME_TYPES);
/** Set view of {@link CHAT_UPLOAD_MIME_TYPES} for O(1) membership checks (lowercase). */
export const CHAT_UPLOAD_MIME_TYPE_SET = new Set(CHAT_UPLOAD_MIME_TYPES);
//# sourceMappingURL=chat-upload-limits.js.map