import { DEFAULT_CACHE_TTL_MS, SELF_STATUS_SCHEMA_VERSION, SUMMARY_CHAR_LIMIT, SUMMARY_TOTAL_CHAR_LIMIT, } from "../contracts/awareness.js";
...
export class AwarenessRegistry {
    ...
    register(contributor) {
        if (this.contributorIds.has(contributor.id)) {
            throw new Error(`AwarenessRegistry: duplicate contributor id "${contributor.id}"`);
        }
        this.contributorIds.add(contributor.id);
        const idx = this.contributors.findIndex((c) => c.position > contributor.position);
        if (idx === -1) {
    ...
    async getDetail(runtime, module, level) {
        if (module === "all") {
    ...
    async getCachedSummary(contributor, runtime) {
    ...
    applyGlobalBudget(lines, header) {
        ...
        if (remaining > 0) {
            let suffix = `\n[+${remaining} more]`;
            while (bodyLen + suffix.length + headerLen + 1 > SUMMARY_TOTAL_CHAR_LIMIT &&
                included.length > 1) {
    ...
    async composeAllDetails(runtime, level) {
        const parts = [];
        for (const contributor of this.contributors) {
            if (!contributor.detail) {
