...
function trimPackagePrefix(value) {
    return value
        .replace(/^@[^/]+\//, "")
        .replace(/^(app|plugin)-/i, "")
        .trim();
}
export function getAppHeroDisplayLabel(app) {
    return trimPackagePrefix(app.displayName ?? app.name);
}
export function getAppHeroMonogram(app) {
...
export function getAppHeroThemeKey(app) {
    const blob = [
        app.name,
        app.displayName ?? "",
        app.category ?? "",
...
function getHeroPalette(name) {
    return HERO_PALETTES[hashString(name) % HERO_PALETTES.length];
}
function getSeededOffset(seed, divisor, spread) {
...
function renderPlayMotif(seed, palette) {
...
function renderChatMotif(seed, palette) {
...
function renderMoneyMotif(seed, palette) {
...
function renderToolsMotif(seed, palette) {
...
function renderWorldMotif(seed, palette) {
...
function renderOpsMotif(seed, palette) {
...
function renderDefaultMotif(seed, palette) {
...
function renderThemeMotif(theme, seed, palette) {
    switch (theme) {
        case "play":
...
export function createGeneratedAppHeroSvg(app) {
...
export function createGeneratedAppHeroDataUrl(app) {
    return `data:image/svg+xml;charset=utf-8,${encodeURIComponent(createGeneratedAppHeroSvg(app))}`;
}
//# sourceMappingURL=app-hero-art.js.map