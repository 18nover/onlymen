...
export function normalizeBrandEnvPrefix(prefix) {
...
export function buildBrandEnvAliases(prefix) {
    const normalizedPrefix = normalizeBrandEnvPrefix(prefix);
    return BRAND_ENV_ALIAS_DEFINITIONS.map((definition) => {
        const brandKey = "vite" in definition && definition.vite
            ? `VITE_${normalizedPrefix}_${definition.brandSuffix}`
            : `${normalizedPrefix}_${definition.brandSuffix}`;
        return [brandKey, definition.elizaKey];
    });
}
export function buildBrandEnvSyncAliases(prefix) {
    const normalizedPrefix = normalizeBrandEnvPrefix(prefix);
    return BRAND_ENV_ALIAS_DEFINITIONS.map((definition) => {
        const brandKey = "vite" in definition && definition.vite
            ? `VITE_${normalizedPrefix}_${definition.brandSuffix}`
            : `${normalizedPrefix}_${definition.brandSuffix}`;
        const elizaKey = "syncElizaKey" in definition
            ? definition.syncElizaKey
            : definition.elizaKey;
        return [brandKey, elizaKey];
    });
}
//# sourceMappingURL=brand-env-aliases.js.map