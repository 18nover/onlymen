 ...
 */
export const DISTRIBUTION_PROFILES = ["store", "unrestricted"];
const DISTRIBUTION_PROFILE_ENV_KEY = "ELIZA_DISTRIBUTION_PROFILE";
export function isDistributionProfile(value) {
    return (typeof value === "string" &&
        DISTRIBUTION_PROFILES.includes(value));
}
export function resolveDistributionProfile(env = process.env) {
    ...
    if (!trimmed)
        return "unrestricted";
    if (isDistributionProfile(trimmed))
        return trimmed;
    throw new Error(`Invalid ${DISTRIBUTION_PROFILE_ENV_KEY}=${raw}. Expected one of: ${DISTRIBUTION_PROFILES.join(", ")}.`);
}
//# sourceMappingURL=distribution-profile.js.map