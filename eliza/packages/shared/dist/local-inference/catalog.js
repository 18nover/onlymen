 ...
 */
import { resolveHfDownloadBases } from "./hf-proxy.js";
export const ELIZA_1_HF_REPO = "elizaos/eliza-1";
...
export const ELIZA_1_RELEASE_TIER_IDS = ELIZA_1_TIER_IDS;
...
const _ELIZA_1_VISION_TIER_ID_SET = new Set(ELIZA_1_VISION_TIER_IDS);
...
function hostedMtpDrafterAvailableForTier(id) {
...
const _ELIZA_1_ON_DEVICE_TIER_ID_SET = new Set(ELIZA_1_ON_DEVICE_TIER_IDS);
export function isOnDeviceTier(id) {
...
export const FIRST_RUN_DEFAULT_MODEL_ID = "eliza-1-2b";
export const DEFAULT_ELIGIBLE_MODEL_IDS = new Set(ELIZA_1_RELEASE_TIER_IDS);
export function isDefaultEligibleId(id) {
 ...
 */
export const ELIZA_1_TIER_PUBLISH_STATUS = {
...
export function eliza1TierPublishStatus(id) {
 ...
 */
export const ELIZA_1_VOICE_BACKENDS = {
    "eliza-1-2b": ["kokoro"],
    "eliza-1-4b": ["kokoro"],
    "eliza-1-9b": ["kokoro"],
    "eliza-1-27b": ["kokoro"],
...
const BASE_REQUIRED_KERNELS = ["turbo3", "turbo4"];
const TIER_SPECS = {
    "eliza-1-2b": {
        id: "eliza-1-2b",
        params: "2B",
...
function tierSlug(id) {
    return id.slice("eliza-1-".length);
}
function tierDisplaySlug(id) {
    switch (id) {
        case "eliza-1-2b":
            return "2B";
        case "eliza-1-4b":
...
function tierDisplayName(id) {
    return `eliza-1-${tierDisplaySlug(id)}`;
}
function bundleRemotePrefix(id) {
    return `bundles/${tierSlug(id)}`;
}
function bundlePath(_id, rel) {
    return rel;
}
function bundleRemotePath(id, rel) {
    return `${bundleRemotePrefix(id)}/${rel}`;
}
function bundleComponent(id, file) {
    return { repo: ELIZA_1_HF_REPO, file: bundleRemotePath(id, file) };
}
function primaryVoiceFileForTier(_id) {
    return "tts/kokoro/kokoro-82m-v1_0.gguf";
}
function asrFileForTier(id) {
    return `asr/mmproj-audio-${tierSlug(id)}-bf16.gguf`;
}
function sourceModelForTier(id) {
    ...
    if (isOnDeviceTier(id)) {
        components.litert = bundleComponent(id, `text/eliza-1-${tierSlug(id)}.litertlm`);
    }
    if (spec.hasEmbedding) {
        components.embedding = bundleComponent(id, "embedding/eliza-1-embedding.gguf");
    }
    if (spec.hasVision) {
        components.vision = bundleComponent(id, `vision/mmproj-${tierSlug(id)}.gguf`);
    }
...
function runtimeForTier(id, contextLength) {
    const requiresKernel = contextLength >= 65536
...
function textQuantizationMatrix(args) {
    ...
    const mk = (id, label, scale, minRamScale, status, extra) => ({
        id,
        label,
        ggufFile: extra?.artifactFormat === "litertlm"
            ? litertFile
            : id === "q4_k_m"
    ...
    const variants = [
        ...
        mk("q4_0", "4-bit", 0.94, 0.95, "planned", args.onDevice ? { mobilePreferred: true } : undefined),
    ...
    if (args.onDevice) {
        variants.push(mk("wna8o8", "4-bit", 0.94, 0.95, "planned", {
            artifactFormat: "litertlm",
        }));
    }
    return { defaultVariantId: "q4_k_m", variants };
}
function blurbForTier(id) {
...
export const MODEL_CATALOG = ELIZA_1_TIER_IDS.map((id) => chatTier(id));
...
export function buildHuggingFaceResolveUrlCandidatesForPath(model, filePath) {
    const cleanFilePath = filePath.replace(/^\/+/, "");
    const cleanPrefix = model.hfPathPrefix?.replace(/^\/+|\/+$/g, "");
    const pathWithPrefix = cleanPrefix &&
        cleanFilePath !== cleanPrefix &&
        !cleanFilePath.startsWith(`${cleanPrefix}/`)
        ? `${cleanPrefix}/${cleanFilePath}`
        : cleanFilePath;
    if (model.hub === "modelscope") {
        const base = process.env.ELIZA_MODELSCOPE_BASE_URL?.trim().replace(/\/+$/, "") ||
