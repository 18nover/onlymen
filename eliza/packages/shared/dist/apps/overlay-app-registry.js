...
function getOverlayRegistry() {
    return getUiRegistryStore(OVERLAY_APP_REGISTRY_STORE, () => new Map());
}
/** Register an overlay app. Call at module scope. */
export function registerOverlayApp(app) {
    getOverlayRegistry().set(app.name, app);
}
/** Look up a registered overlay app by name. */
export function getOverlayApp(name) {
    return getOverlayRegistry().get(name);
}
/** Get all registered overlay apps. */
export function getAllOverlayApps() {
    return Array.from(getOverlayRegistry().values());
}
export function getAvailableOverlayApps(context = detectOverlayAvailabilityContext()) {
    const availability = typeof context === "string"
    ...
    return getAllOverlayApps().filter((app) => canShowAndroidOnly || app.androidOnly !== true);
}
function normalizeOverlayAvailabilityContext(context) {
    ...
    return {
        platform,
        aospAndroid: context.aospAndroid ??
 ...
 */
export function isAospAndroid(context = {}) {
...
//# sourceMappingURL=overlay-app-registry.js.map