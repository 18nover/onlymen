 ...
 * `process.env`.
 */
export function isSelfEditEnabled(env = readProcessEnv()) {
    if (!isTruthyEnvValue(env[SELF_EDIT_ENABLE_ENV]))
        return false;
 ...
 */
const DENIED_RELATIVE_SUFFIXES = [
 ...
 */
export function isSelfEditPathDenied(absolutePath) {
    ...
    if (containsGitDirSegment(normalized))
        return true;
    for (const suffix of DENIED_RELATIVE_SUFFIXES) {
        if (pathEndsWithSegment(normalized, suffix))
            return true;
    }
    return false;
}
 ...
 */
export function getSelfEditDeniedSuffixes() {
...
function containsGitDirSegment(normalized) {
...
function pathEndsWithSegment(normalized, suffix) {
...
//# sourceMappingURL=self-edit.js.map