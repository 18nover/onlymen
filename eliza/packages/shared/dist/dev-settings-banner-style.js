...
function shouldSkipBannerColor() {
...
function colorizeDevSettingsBannerLine(line) {
    if (line.length === 0)
        return line;
    ...
    if (c0 === 0x251c /* ├ */)
        return `${DIM_CYAN}${line}${RESET}`;
    if (c0 === 0x2502 /* │ */)
        return `${DIM_CYAN}${line}${RESET}`;
    return line;
}
/** Add cyan emphasis to Unicode box lines; returns input unchanged when color is disabled. */
export function colorizeDevSettingsBanner(text) {
    if (shouldSkipBannerColor())
        return text;
 ...
 */
export function colorizeDevSettingsStartupBanner(text) {
    if (shouldSkipBannerColor())
        return text;
    const idx = text.indexOf("╭");
    if (idx === -1)
        return colorizeDevSettingsBanner(text);
...
//# sourceMappingURL=dev-settings-banner-style.js.map