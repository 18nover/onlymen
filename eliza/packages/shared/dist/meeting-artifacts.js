 ...
 */
export const MEETING_ARTIFACT_SCHEMA_VERSION = "eliza.meeting_artifact.v1";
...
function collectIds(rows, path, errors) {
    const ids = new Set();
    rows.forEach((row, index) => {
        ...
        if (ids.has(row.id))
            errors.push(`duplicate ${path} id: ${row.id}`);
        ids.add(row.id);
    });
    return ids;
}
...
function mediaUrlIsContentAddressed(url) {
    return /^\/api\/media\/[a-f0-9]{16,64}\.[a-z0-9]+$/i.test(url);
}
export function validateMeetingArtifact(value) {
    const errors = [];
    if (!isRecord(value))
    ...
    if (!isRecord(meeting)) {
        ...
        if (!isNonEmptyString(meeting.id))
            errors.push("meeting.id is required");
        requireEnum(meeting.captureMode, MEETING_ARTIFACT_CAPTURE_MODES, "meeting.captureMode", errors);
        ...
        if (!isRecord(consent)) {
            ...
            requireEnum(consent.state, ["unknown", "granted", "denied", "not_required", "redacted"], "meeting.consent.state", errors);
        }
    ...
    const participantIds = collectIds(participants, "platformParticipants", errors);
    ...
    media.forEach((row, index) => {
        ...
        if (!isNonEmptyString(row.url) || !mediaUrlIsContentAddressed(row.url)) {
            errors.push(`media[${index}].url must be a content-addressed /api/media URL`);
        }
    ...
    streams.forEach((row, index) => {
        if (!isRecord(row))
            return;
        requireEnum(row.kind, MEETING_ARTIFACT_SOURCE_STREAM_KINDS, `sourceStreams[${index}].kind`, errors);
        if (!isNonEmptyString(row.mediaRefId) || !mediaIds.has(row.mediaRefId)) {
            errors.push(`sourceStreams[${index}].mediaRefId references missing media`);
        }
        if (row.platformParticipantId !== undefined &&
            (!isNonEmptyString(row.platformParticipantId) ||
                !participantIds.has(row.platformParticipantId))) {
            errors.push(`sourceStreams[${index}].platformParticipantId references missing participant`);
        }
    });
    speakers.forEach((row, index) => {
        if (!isRecord(row))
            return;
        requireRefs(row.sourceStreamIds, streamIds, `diarizedSpeakers[${index}].sourceStreamIds`, errors);
        if (row.platformParticipantIds !== undefined) {
            requireRefs(row.platformParticipantIds, participantIds, `diarizedSpeakers[${index}].platformParticipantIds`, errors);
        }
        if (row.entityBindingId !== undefined &&
            (!isNonEmptyString(row.entityBindingId) ||
                !bindingIds.has(row.entityBindingId))) {
            errors.push(`diarizedSpeakers[${index}].entityBindingId references missing binding`);
        }
        const name = row.name;
        if (name !== undefined) {
            if (!isRecord(name)) {
                ...
                requireEnum(name.provenance, MEETING_SPEAKER_NAME_PROVENANCE, `diarizedSpeakers[${index}].name.provenance`, errors);
                if (!isNumber(name.confidence) ||
                    name.confidence < 0 ||
                    name.confidence > 1) {
                    errors.push(`diarizedSpeakers[${index}].name.confidence must be in [0,1]`);
                }
            }
        }
    });
    bindings.forEach((row, index) => {
        if (!isRecord(row))
            return;
        if (!isNonEmptyString(row.diarizedSpeakerId) ||
            !speakerIds.has(row.diarizedSpeakerId)) {
            errors.push(`entityBindings[${index}].diarizedSpeakerId references missing speaker`);
        }
        requireEnum(row.status, ["active", "merged", "split", "deleted", "revoked", "unknown"], `entityBindings[${index}].status`, errors);
        requireEnum(row.provenance, MEETING_SPEAKER_NAME_PROVENANCE, `entityBindings[${index}].provenance`, errors);
    ...
    spans.forEach((row, index) => {
        ...
        if (!isNonEmptyString(row.sourceStreamId) ||
            !streamIds.has(row.sourceStreamId)) {
            errors.push(`transcriptSpans[${index}].sourceStreamId references missing stream`);
        }
        if (row.speakerId !== undefined &&
            (!isNonEmptyString(row.speakerId) || !speakerIds.has(row.speakerId))) {
            errors.push(`transcriptSpans[${index}].speakerId references missing speaker`);
        }
        if (row.platformParticipantId !== undefined &&
            (!isNonEmptyString(row.platformParticipantId) ||
                !participantIds.has(row.platformParticipantId))) {
            errors.push(`transcriptSpans[${index}].platformParticipantId references missing participant`);
        }
        ...
        words.forEach((word, wordIndex) => {
            if (!isRecord(word)) {
                errors.push(`transcriptSpans[${index}].words[${wordIndex}] must be an object`);
                return;
            }
            if (!isNonEmptyString(word.text)) {
                errors.push(`transcriptSpans[${index}].words[${wordIndex}].text is required`);
            }
            requireTimeRange(word, `transcriptSpans[${index}].words[${wordIndex}]`, errors);
        });
    });
    for (const key of ["notes", "actionItems", "decisions"]) {
        const rows = Array.isArray(value[key]) ? value[key] : [];
        rows.forEach((row, index) => {
            if (!isRecord(row))
                return;
            requireRefs(row.transcriptSpanIds, spanIds, `${key}[${index}].transcriptSpanIds`, errors);
        });
    }
    ...
    evidenceRows.forEach((row, index) => {
        if (!isRecord(row))
            return;
        if (row.mediaRefId !== undefined &&
            (!isNonEmptyString(row.mediaRefId) || !mediaIds.has(row.mediaRefId))) {
            errors.push(`evidenceArtifacts[${index}].mediaRefId references missing media`);
        }
        if (row.transcriptSpanIds !== undefined) {
            requireRefs(row.transcriptSpanIds, spanIds, `evidenceArtifacts[${index}].transcriptSpanIds`, errors);
        }
    });
    return { valid: errors.length === 0, errors };
}
export function assertValidMeetingArtifact(value) {
    const validation = validateMeetingArtifact(value);
    if (!validation.valid) {
        throw new Error(`invalid meeting artifact: ${validation.errors.join("; ")}`);
    }
}
export function meetingArtifactToTranscriptSegments(artifact) {
    return artifact.transcriptSpans.map((span) => {
        const speaker = artifact.diarizedSpeakers.find((candidate) => candidate.id === span.speakerId);
        return {
            id: span.id,
            speakerLabel: speaker?.name?.displayName ?? span.speakerId,
            speakerEntityId: speaker?.entityBindingId
                ? (artifact.entityBindings.find((binding) => binding.id === speaker.entityBindingId)?.entityId ?? undefined)
                : undefined,
...
function media(id, mimeType = "audio/wav") {
    return {
        id,
...
export function buildMeetingArtifactFixtures() {
...
//# sourceMappingURL=meeting-artifacts.js.map