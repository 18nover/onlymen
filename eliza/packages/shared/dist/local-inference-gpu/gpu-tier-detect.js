...
export function detectNvidiaGpu() {
    try {
        const raw = execSync("nvidia-smi --query-gpu=name,memory.total,compute_cap --format=csv,noheader,nounits", { encoding: "utf8", timeout: 2_000, stdio: ["ignore", "pipe", "pipe"] });
        ...
        if (Number.isNaN(vram_mb) || vram_mb <= 0)
            return null;
        // compute_cap may be "N/A" on very old drivers.
        const cuda_compute = computeRaw.length > 0 && computeRaw !== "N/A" ? computeRaw : null;
 ...
 * Never throws.
 */
export function autoSelectProfile() {
...
//# sourceMappingURL=gpu-tier-detect.js.map