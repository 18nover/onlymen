 ...
 */
export function resolveKmsBackend(opts = {}, env = opts.env ?? process.env) {
    if (opts.backend)
        return opts.backend;
 ...
 */
function decodeRootKey(b64, source) {
    ...
    if (pad === 2)
        ...
        throw new KmsError(`${source} is not valid base64 (invalid length after normalization)`);
    }
    let buf;
    try {
        ...
        throw new KmsError(`${source} failed base64 decode: ${cause.message}`);
    }
    if (buf.length !== 32) {
        throw new KmsError(`${source} must decode to 32 bytes (got ${buf.length}); generate one with ` +
 ...
 */
function resolveLocalRootKey(opts, env) {
    if (opts.local?.rootKey)
        return opts.local.rootKey;
    ...
    throw new KmsError("LocalKmsAdapter requires a persistent root key; set ELIZA_LOCAL_ROOT_KEY " +
        "(base64-encoded 32 bytes) or pass opts.local.rootKey. Generate with: " +
        "`openssl rand -base64 32`");
}
export function createKmsClient(opts = {}) {
    ...
    if (backend === "steward" && !opts.steward) {
        ...
        if (localKey && localKey.length > 0) {
            ...
            console.warn("[kms] ELIZA_KMS_BACKEND=steward selected but steward.{baseUrl,tokenProvider} " +
    ...
    switch (backend) {
        ...
        case "steward": {
            ...
            if (!cfg) {
                throw new KmsError("ELIZA_KMS_BACKEND=steward requires steward.{baseUrl, tokenProvider}");
            }
