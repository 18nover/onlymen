 ...
 */
export function hkdfSha256(ikm, length, info = new Uint8Array(0), salt = new Uint8Array(0)) {
    if (length <= 0 || length > 255 * 32) {
        throw new Error(`hkdf length out of range: ${length}`);
    }
    const out = hkdfSync("sha256", Buffer.from(ikm), Buffer.from(salt), Buffer.from(info), length);
