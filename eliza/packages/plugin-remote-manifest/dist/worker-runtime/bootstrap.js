 ...
 */
import { buildAnnounceDescriptor, createHandlerRegistry, } from "./descriptor.js";
import { createWorkerRpcDispatcher } from "./dispatch.js";
import { createDefaultChannel, createRequestIdAllocator, } from "./envelope.js";
import { toWireError } from "./error.js";
import { buildRuntimeProxyApi, RuntimeProxy } from "./runtime-proxy.js";
function snapshotPluginSurfaces(plugin) {
    return {
        ...
        events: new Map(Object.entries(plugin.events ?? {}).map(([eventName, handlers]) => [
            eventName,
            new Set(handlers),
        ])),
    };
}
function appended(items, seen) {
    return (items ?? []).filter((item) => !seen.has(item));
}
function buildDynamicPluginShape(plugin, snapshot) {
    const dynamic = { name: plugin.name };
    ...
    addArray("componentTypes", appended(plugin.componentTypes, snapshot.componentTypes));
    const models = Object.fromEntries(Object.entries(plugin.models ?? {}).filter(([modelType]) => !snapshot.modelKeys.has(modelType)));
    ...
    for (const [eventName, handlers] of Object.entries(plugin.events ?? {})) {
        ...
        const dynamicHandlers = handlers.filter((handler) => !existingHandlers.has(handler));
 ...
 */
export async function bootstrap(plugin, options = {}) {
    ...
    channel.onMessage((message) => {
        if (message.type === "worker-rpc") {
            void dispatchRpc(message);
        }
    });
    // Build + send the announce payload.
    const descriptor = buildAnnounceDescriptor(plugin, registry);
    const announce = {
    ...
    if (typeof plugin.init === "function") {
        try {
            await plugin.init(options.initConfig ?? {}, runtimeApi);
        }
        catch (error) {
            channel.send({
                type: "event",
                ...
                payload: {
                    error: JSON.parse(JSON.stringify(toWireError(error))),
                },
            });
            throw error;
        }
