...
export declare const SUPPORTED_RUNTIME_METHODS: readonly ["getService", "useModel", "getMemory", "createMemory", "updateMemory", "emitEvent", "getSetting", "setSetting", "composeState", "actionCallback"];
...
export interface RuntimeProxyApi {
    ...
    actionCallback(callbackId: string, response: JsonValue, actionName?: string): Promise<JsonValue>;
}
export interface BuildRuntimeProxyApiOptions {
     ...
     */
    registerDynamicEventHandler?: (name: string, handler: (payload: JsonValue) => void) => {
        rpc: true;
        id: string;
    };
}
export declare function buildRuntimeProxyApi(proxy: RuntimeProxy, options?: BuildRuntimeProxyApiOptions): RuntimeProxyApi;
