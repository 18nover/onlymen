...
export function installAndroidShim(options = {}) {
    if (installedAndroidShim)
        return installedAndroidShim;
    const bridge = window.ElizaosAndroidBridge;
    if (!bridge) {
        throw new Error("installAndroidShim(): window.ElizaosAndroidBridge missing — " +
    ...
    let nextId = 0;
    window.__elizaosAndroidDeliver = (json) => {
        let data;
        try {
    ...
    const shim = {
        resolveViewUrl(pluginName, relativePath) {
            ...
            return new URL(`https://appassets.androidplatform.net/plugins/${encodeURIComponent(pluginName)}/${safeRelativePath}`);
        },
        request(method, params) {
            ...
            return new Promise((resolve, reject) => {
                ...
                try {
                    bridge.postMessage(JSON.stringify({ kind: "request", id, method, params }));
                }
                catch (cause) {
                    pending.delete(id);
...
let installedAndroidShim = null;
let installedAndroidShimCleanup = null;
export function resetAndroidShimForTests() {
    installedAndroidShimCleanup?.();
