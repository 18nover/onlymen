...
export function installWebShim(options = {}) {
    ...
    const onMessage = (event) => {
        ...
        if (isParentResponse(message)) {
            ...
            if (message.ok) {
                slot.resolve((message.payload ?? null));
            }
            else {
    ...
    const shim = {
        resolveViewUrl(pluginName, relativePath) {
            ...
            return new URL(`${viewsBasePath}/${encodeURIComponent(pluginName)}/${safeRelativePath}`, window.location.href);
        },
...
let installedWebShim = null;
let installedWebShimCleanup = null;
export function resetWebShimForTests() {
    installedWebShimCleanup?.();
...
function isParentResponse(message) {
    if (typeof message !== "object" || message === null)
        return false;
    const candidate = message;
    return (candidate.kind === "elizaos.shim.response" &&
        typeof candidate.id === "number" &&
        Number.isFinite(candidate.id) &&
        typeof candidate.ok === "boolean" &&
        (candidate.error === undefined || typeof candidate.error === "string"));
}
function isParentEvent(message) {
    if (typeof message !== "object" || message === null)
        return false;
    const candidate = message;
    return (candidate.kind === "elizaos.shim.event" &&
