...
const RETENTION_DAYS = Number.parseInt(process.env.ELIZA_SUB_AGENT_SESSION_RETENTION_DAYS ?? "30", 10);
/**
...
function redactEmails(line) {
    ...
    while (i < line.length) {
        ...
        if (at === -1)
            break;
        let start = at - 1;
        while (start >= 0 && isEmailAtomChar(line.charCodeAt(start)))
            start -= 1;
        start += 1;
        let end = at + 1;
        while (end < line.length && isDomainChar(line.charCodeAt(end)))
            end += 1;
        while (end > at + 1) {
...
export function redactTranscriptLine(line) {
...
export class SessionRecorder {
    ...
    record(line) {
        ...
        try {
            ...
            process.stderr.write(`[sub-agent] WARN: session ${this.opts.sessionId} transcript write failed; audit record will report an incomplete transcript: ${this.writeFailure.message}\n`);
        }
    }
    async finalize() {
        if (this.finalized)
        ...
        if (this.opts.auditDispatcher) {
            ...
            try {
                ...
                process.stderr.write(`[sub-agent] WARN: session ${this.opts.sessionId} audit emit failed; session_record event dropped: ${cause.message}\n`);
            }
        }
    }
}
 ...
 */
export function pruneOldSessions(now = Date.now(), sessionsRoot = SESSIONS_ROOT) {
    ...
    for (const entry of readdirSync(sessionsRoot, { withFileTypes: true })) {
        ...
        try {
            ...
            const cause = error instanceof Error ? error : new Error(String(error));
            process.stderr.write(`[sub-agent] WARN: retention prune skipped ${dir}: ${cause.message}\n`);
        }
    }
    return removed;
}
