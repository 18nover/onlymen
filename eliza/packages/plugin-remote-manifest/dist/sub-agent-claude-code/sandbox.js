 ...
 */
const SENSITIVE_ENV_RE = /(TOKEN|SECRET|KEY|PASSWORD|CREDENTIAL|DATABASE_URL|WALLET|PRIVATE|MNEMONIC|API_KEY)/i;
/** Whitelisted absolute directories that may host the sub-agent binary. */
const BINARY_DIR_ALLOWLIST = [
    "/usr/local/bin",
    "/usr/bin",
    "/opt/homebrew/bin",
    `${homedir()}/.local/bin`,
...
export function filterEnv(source, allow = SAFE_ENV_KEYS, extra = {}) {
    const out = {};
 ...
 */
export function resolveSafeCwd(cwd, workspaceRoots) {
    ...
    throw new SubAgentCwdError(`cwd ${real} is not under any allowed workspace root (${candidates.join(", ")})`);
}
 ...
 */
export function resolveSafeBinary(binary, env = process.env) {
    if (!binary)
        throw new SubAgentBinaryError("binary is required");
    ...
    if (isAbsolute(binary)) {
        ...
        for (const dir of pathDirs) {
            ...
            if (!candidateDirs.includes(real))
                continue;
    ...
    if (!resolved || !existsSync(resolved)) {
        throw new SubAgentBinaryError(`Could not resolve ${binary} under any whitelisted dir: ${candidateDirs.join(", ")}`);
    }
    const real = realpathSync(resolved);
    const realDir = dirname(real);
    if (!candidateDirs.some((d) => realDir === d || realDir.startsWith(d + sep))) {
        throw new SubAgentBinaryError(`Resolved binary ${real} is not under a whitelisted dir`);
    }
    return real;
}
/**
 ...
 */
export function buildSandboxedCommand(argv, opts) {
    if (process.platform === "darwin") {
        if (opts.macosProfile &&
 ...
 */
export function locateBundledProfiles(packageRoot) {
