...
export function isValidRemotePluginId(value) {
...
function optionalBooleanAt(value, path, issues) {
    if (value === undefined)
        return undefined;
...
function numberAt(object, key, path, issues) {
    const value = object[key];
...
function optionalStringAt(object, key, path, issues) {
    const value = object[key];
    if (value === undefined)
        return undefined;
...
function validateMode(value, issues) {
    if (value === "window" || value === "background")
        return value;
    issues.push({ path: "mode", message: "Expected window or background." });
    return null;
}
function validateBooleanRecord(value, path, allowed, issues) {
    if (value === undefined)
        return undefined;
...
function validatePermissions(value, issues) {
    ...
    if (!object)
        return null;
    const host = validateBooleanRecord(object.host, "permissions.host", HOST_PERMISSIONS, issues);
    const bun = validateBooleanRecord(object.bun, "permissions.bun", BUN_PERMISSIONS, issues);
    const grant = {};
    if (host)
        grant.host = host;
    if (bun)
...
function validateRemoteUIs(value, issues) {
    if (value === undefined)
        return undefined;
...
function validateView(value, issues) {
    ...
    const titleBarStyle = optionalStringAt(object, "titleBarStyle", "view", issues);
    ...
    const transparent = optionalBooleanAt(object.transparent, "view.transparent", issues);
    if (!relativePath || !title || width === null || height === null)
        return null;
    return {
        relativePath,
        title,
...
function validateWorker(value, issues) {
...
export function validateRemotePluginManifest(value) {
    const issues = [];
    ...
    if (id && !isValidRemotePluginId(id)) {
        issues.push({
            path: "$.id",
            message: "Expected remote plugin id segments containing only letters, numbers, underscores, hyphens, or dots.",
        });
    }
    ...
    const dependencies = validateStringMap(object.dependencies, "dependencies", issues);
