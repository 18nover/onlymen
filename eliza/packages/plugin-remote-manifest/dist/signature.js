...
export async function verifyPluginArtifact(input) {
    ...
    const emitFailure = async (reason) => {
        if (!auditDispatcher)
            return;
    ...
    if (!signature.hash) {
        ...
        throw new PluginSignatureError(`Plugin ${pluginId} missing required SHA-256 hash`, "MISSING_HASH");
    }
    if (!signature.signature) {
        await emitFailure("missing_signature");
        throw new PluginSignatureError(`Plugin ${pluginId} missing required Ed25519 signature`, "MISSING_SIGNATURE");
    }
    ...
    if (expectedHash !== actualHash) {
        ...
        throw new PluginSignatureError(`Plugin ${pluginId} hash mismatch: expected ${expectedHash}, got ${actualHash}`, "HASH_MISMATCH");
    }
    const hashBytes = fromHex(actualHash);
    const sigBytes = fromBase64(signature.signature);
    const ok = await kms.verify(PLUGIN_MANIFEST_KEY, hashBytes, sigBytes, "ed25519");
    if (!ok) {
        await emitFailure("bad_signature");
        throw new PluginSignatureError(`Plugin ${pluginId} Ed25519 signature verification failed`, "BAD_SIGNATURE");
    }
    if (auditDispatcher) {
        await auditDispatcher.emit({
            actor: { type: actorId ? "user" : "system", id: actorId ?? "agent" },
