 ...
 */
import type { AccessContext, Agent, AppendConnectorAccountAuditEventParams, Component, ConnectorAccountAuditEventRecord, ConnectorAccountCredentialRefRecord, ConnectorAccountRecord, ConsumeOAuthFlowStateParams, CreateOAuthFlowStateParams, DeleteConnectorAccountParams, DeleteOAuthFlowStateParams, Entity, GetConnectorAccountCredentialRefParams, GetConnectorAccountParams, GetOAuthFlowStateParams, IDatabaseAdapter, JsonValue, ListConnectorAccountCredentialRefsParams, ListConnectorAccountsParams, Log, LogBody, Memory, MemoryMetadata, MessageSearchHit, Metadata, OAuthFlowRecord, PairingAllowlistEntry, PairingChannel, PairingRequest, Participant, ParticipantUpdateFields, ParticipantUserState, PatchOp, Relationship, Room, SetConnectorAccountCredentialRefParams, Task, UpdateOAuthFlowStateParams, UpsertConnectorAccountParams, UUID, World } from "./types/index.js";
/**
 ...
 * @abstract
 */
export declare abstract class DatabaseAdapter<DB extends object = object> implements IDatabaseAdapter<DB> {
    /**
    ...
    abstract getComponentsForEntities(entityIds: UUID[], worldId?: UUID, sourceEntityId?: UUID): Promise<Component[]>;
     ...
     * @param options.entityContext When set (Postgres + ENABLE_DATA_ISOLATION), runs under RLS for this entity.
     */
    abstract upsertComponents(components: Component[], options?: {
        entityContext?: UUID;
    }): Promise<void>;
    abstract patchComponents(updates: Array<{
        componentId: UUID;
        ops: PatchOp[];
    }>, options?: {
        entityContext?: UUID;
    ...
    abstract createLogs(params: Array<{
        body: LogBody;
        entityId: UUID;
        roomId: UUID;
        type: string;
    ...
    abstract updateLogs(logs: Array<{
        id: UUID;
        updates: Partial<Log>;
    }>): Promise<void>;
    ...
    abstract createMemories(memories: Array<{
        memory: Memory;
        tableName: string;
        unique?: boolean;
    }>): Promise<UUID[]>;
    abstract updateMemories(memories: Array<Partial<Memory> & {
        id: UUID;
     ...
     * @param options.entityContext When set (Postgres + ENABLE_DATA_ISOLATION), runs under RLS for this entity.
     */
    abstract upsertMemories(memories: Array<{
        memory: Memory;
        tableName: string;
    }>, options?: {
        entityContext?: UUID;
     ...
     */
    abstract createRoomParticipants(entityIds: UUID[], roomId: UUID): Promise<UUID[]>;
    /** WHY boolean: Callers need success/failure for error handling/UX; see IDatabaseAdapter in types/database.ts. */
    abstract deleteParticipants(participants: Array<{
        entityId: UUID;
        roomId: UUID;
    }>): Promise<boolean>;
    ...
    abstract getParticipantUserStates(pairs: Array<{
        roomId: UUID;
        entityId: UUID;
    ...
    abstract getRelationshipsByPairs(pairs: Array<{
        sourceEntityId: UUID;
        targetEntityId: UUID;
    ...
    abstract updateAgents(updates: Array<{
        agentId: UUID;
        agent: Partial<Agent>;
    }>): Promise<boolean>;
    ...
    abstract setCaches<T>(entries: Array<{
        key: string;
        value: T;
    }>): Promise<boolean>;
    ...
    abstract updateTasks(updates: Array<{
        id: UUID;
        task: Partial<Task>;
    }>): Promise<void>;
    ...
    listConnectorAccounts(_params?: ListConnectorAccountsParams): Promise<ConnectorAccountRecord[]>;
    getConnectorAccount(_params: GetConnectorAccountParams): Promise<ConnectorAccountRecord | null>;
    upsertConnectorAccount(_params: UpsertConnectorAccountParams): Promise<ConnectorAccountRecord>;
    deleteConnectorAccount(_params: DeleteConnectorAccountParams): Promise<boolean>;
    setConnectorAccountCredentialRef(_params: SetConnectorAccountCredentialRefParams): Promise<ConnectorAccountCredentialRefRecord>;
    getConnectorAccountCredentialRef(_params: GetConnectorAccountCredentialRefParams): Promise<ConnectorAccountCredentialRefRecord | null>;
    listConnectorAccountCredentialRefs(_params: ListConnectorAccountCredentialRefsParams): Promise<ConnectorAccountCredentialRefRecord[]>;
    appendConnectorAccountAuditEvent(_params: AppendConnectorAccountAuditEventParams): Promise<ConnectorAccountAuditEventRecord>;
    createOAuthFlowState(_params: CreateOAuthFlowStateParams): Promise<OAuthFlowRecord>;
    consumeOAuthFlowState(_params: ConsumeOAuthFlowStateParams): Promise<OAuthFlowRecord | null>;
    getOAuthFlowState(_params: GetOAuthFlowStateParams): Promise<OAuthFlowRecord | null>;
    updateOAuthFlowState(_params: UpdateOAuthFlowStateParams): Promise<OAuthFlowRecord | null>;
