 ...
 */
import type { ActionResult, HandlerCallback } from "./components.js";
import type { Memory } from "./memory.js";
import type { JSONSchema, ModelTypeName } from "./model.js";
...
export interface EvaluatorPromptContext<TPrepared = unknown> extends EvaluatorRunContext {
    state: State;
    prepared: TPrepared;
}
export interface EvaluatorProcessorContext<TOutput = JsonValue, TPrepared = unknown> extends EvaluatorPromptContext<TPrepared> {
    output: TOutput;
...
export interface EvaluatorProcessor<TOutput = JsonValue, TPrepared = unknown> {
    name?: string;
    priority?: number;
    process(context: EvaluatorProcessorContext<TOutput, TPrepared>): Promise<ActionResult | undefined>;
}
export interface Evaluator<TOutput = JsonValue, TPrepared = unknown> {
    ...
    prepare?(context: EvaluatorRunContext & {
        state: State;
    }): Promise<TPrepared>;
